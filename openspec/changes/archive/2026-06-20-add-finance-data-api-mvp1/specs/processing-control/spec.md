...
The system SHALL provide a pending work query that lists unprocessed news, unreviewed operations, and missing or stale snapshot indicators.

#### Scenario: List pending processing items
- **WHEN** an agent requests pending processing items
- **THEN** the system returns the pending items grouped by type with stable identifiers
